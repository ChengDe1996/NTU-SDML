...
def collect_words(dataset, source, target):
	"""get word set"""
	split_word_sen = []
	source_split = []
	target_split = []
	...
	for i in range(len(source)):
		...
		words.update(source_temp)
		words.update(target_temp)
...
def main():
	...
	source, target, ctr_list = dele_long(source, target, ctr_list)
	ctr_list = np.array(ctr_list)
	word_sets, split_word_sen, source_split, target_split= collect_words(source_dataset, source, target)
	#print(len(word_sets))
	...
	source_sen_len, target_sen_len = calc_sen_len(source, target)
	max_pad_len = max(source_sen_len + target_sen_len)
